...
class res_50_1(nn.Module):
    def __init__(self,
                 args,
                 act_func=F.leaky_relu,
                 norm=nn.BatchNorm2d):
        super(res_50_1, self).__init__()
        ...
        if self.net_rend_head or self.net_simply_head:
            ...
            self.rend_head_2 = nn.Linear(in_features=args['frames']*embed_dim, out_features=embed_dim, bias=True)

            self.mlp_1 = Mlp(in_features=embed_dim, out_features=embed_dim)
            self.mlp_2 = Mlp(in_features=embed_dim, out_features=embed_dim)
            self.mlp_3 = Mlp(in_features=embed_dim, out_features=embed_dim)
            ...
            self.rend_out_diff = nn.Linear(in_features=embed_dim, 
                                           out_features=args['frames']*self.n_feat_eye_diff, bias=True)
    ...
    def forward(self, data_dict, args):
        ...
        if self.net_rend_head or self.net_simply_head:
            ...
            eye_3d_out = rearrange(eye_3d_out, 'B N C-> B (N C)')
            eye_3d_out = self.rend_head_2(eye_3d_out)
            
            eye_3d_out = self.mlp_1(eye_3d_out)
            eye_3d_out = self.mlp_2(eye_3d_out)
            eye_3d_out = self.mlp_3(eye_3d_out)

            eye_3d_out_same = self.rend_out_same(eye_3d_out).unsqueeze(1)

            if self.net_simply_head:
                eye_3d_out_same = eye_3d_out_same.expand(-1,N,-1)
                eye_3d_out_same = rearrange(eye_3d_out_same, 'B N d -> (B N) d')
            eye_3d_out_diff = self.rend_out_diff(eye_3d_out)
            eye_3d_out_diff = rearrange(eye_3d_out_diff, 'B (N o) -> B N o', N=N)
