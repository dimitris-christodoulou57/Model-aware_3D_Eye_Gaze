...
class DenseNet3_gaze(nn.Module):
    def __init__(self,
                 args,
                 act_func=F.leaky_relu,
                 norm=nn.BatchNorm2d):
        super(DenseNet3_gaze, self).__init__()
        ...
        if args['net_simply_head']:
            embed_dim=256
            self.rend_head_1 = nn.Linear(in_features=153, out_features=embed_dim, bias=True)
    ...
    def forward(self, data_dict, args):
        ...
        if args['net_simply_head']:
            ...
            eye_3d_out_same = torch.relu(eye_3d_out_same)
            eye_3d_out_diff = torch.relu(eye_3d_out_diff)

            out_dict['eyeball_c_UV'] = eye_3d_out_same

            out_dict['pupil_c_UV'] = eye_3d_out_diff[..., :2]
            out_dict['gaze_vector_3D'] = eye_3d_out_diff[..., -3:]
