...
from models.denseEl1.utils import get_com
from models.denseEl1.utils import renderingregressionModule    
from models.denseEl1.utils import eye_param_same_regressionModule  
from models.denseEl1.utils import detach_cpu_np, conv_layer, get_nparams
from models.denseEl1.utils import regressionModule, convBlock

from models.denseEl1.helper import plot_images_with_annotations
from models.denseEl1.helper import construct_mask_from_ellipse
from models.denseEl1.helper import fix_ellipse_axis_angle
from models.denseEl1.helper import my_ellipse
...
class Transition_down(nn.Module):
    ...
    def __init__(self, down_size):
        ...
        self.pool = nn.AvgPool3d(kernel_size=down_size) if down_size else False
...
class DenseNet3D_up_block(nn.Module):
    '''
    ...
    def __init__(self, skip_c, in_c, out_c,
        ...
        super(DenseNet3D_up_block, self).__init__()
...
class DenseNet_encoder(nn.Module):
    def __init__(self, args, in_c=1,
        ...
        for block_num in range(args['num_blocks']):
            block = DenseNet3D_down_block(in_c=ipSize[block_num],
...
class DenseNet_decoder(nn.Module):
    def __init__(self, args, out_c,
        ...
        for block_num in range(args['num_blocks']):
            block = DenseNet3D_up_block(skipSize[block_num],
                                        ipSize[block_num],
                                        opSize[block_num],
                                        (1,2,2), act_func, norm,
                                        groups=args['groups'],
        ...
        self.final = nn.Conv3d(opSize[-1], out_c, kernel_size=1, bias=True)
...
class DenseNet3D(nn.Module):
    def __init__(self,
        ...
        if args['maxpool_in_regress_mod'] > 0:
            regress_pool = nn.MaxPool3d
        elif args['maxpool_in_regress_mod'] == 0:
            regress_pool = nn.AvgPool3d
        else:
            regress_pool = False
    ...
    def setDatasetInfo(self, numSets=2):
        # Produces a 1x1 conv3D layer which directly maps bottleneck to the
        ...
        self.dsIdentify_lin = nn.Conv3d(in_channels=inChannels,
