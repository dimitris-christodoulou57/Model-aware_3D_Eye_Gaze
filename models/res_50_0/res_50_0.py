...
class res_50_0(nn.Module):
    def __init__(self,
                 args,
                 act_func=F.leaky_relu,
                 norm=nn.BatchNorm2d):
        super(res_50_0, self).__init__()
        ...
        self.enc = resnet50(pretrained=args['pretrained_resnet'], **{'in_chans': args['frames']*args['batch_size'] }) # TODO enable parametrizing pretrained weights
        ...
        if self.net_rend_head or self.net_simply_head:
            ...
            N = args['batch_size'] * args['frames']
            if self.net_simply_head:
                self.n_feat_eye_diff = 5 * N
                self.n_feat_eye_same = 2 * args['batch_size']
            elif self.net_rend_head:
                self.n_feat_eye_diff = 7 * N
                self.n_feat_eye_same = 7 * args['batch_size']
            else:
    ...
    def forward(self, data_dict, args):
        ...
        x = rearrange(x, 'B N H W -> 1 (B N) H W')
        start = time.time()
        ...
        if self.net_rend_head or self.net_simply_head:
            ...
            eye_3d_out_diff = rearrange(eye_3d_out_diff, '1 1 (B N F) -> (B N) F', B=B, N=N)

            if self.net_rend_head:
                eye_3d_out_diff = rearrange(eye_3d_out_diff, '(B N) F -> B N F', B=B, N=N)
