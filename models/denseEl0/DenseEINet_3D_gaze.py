...
class DenseNet3D_gaze(nn.Module):
    def __init__(self,
                 args,
                 act_func=F.leaky_relu,
                 norm=nn.BatchNorm2d):
        super(DenseNet3D_gaze, self).__init__()
    ...
    def forward(self, data_dict, args):
        ...
        if args['net_simply_head']:
            ...
            rendOut_same = rendOut_same.unsqueeze(1).expand(-1,NUM_OF_FRAME,-1)
            rendOut_same = rendOut_same.reshape(B*NUM_OF_FRAME, -1)
            ...
            out_dict['eyeball_c_UV'] = rendOut_same

            out_dict['pupil_c_UV'] = rendOut_frames
