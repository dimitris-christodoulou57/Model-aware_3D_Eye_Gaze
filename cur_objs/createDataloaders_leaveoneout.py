...
for setSel in list_ds:
    ...
    # Train object
    AllDS_cond = CurLib.rmDataset(AllDS, setSel)
    dataDiv_obj = CurLib.generate_fileList(AllDS_cond,
                                           mode='vanilla',
                                           notest=True)
    trainObj = DataLoader_riteyes(dataDiv_obj, path2h5,
                                  'train', True, (480, 640), scale=0.5)
    validObj = DataLoader_riteyes(dataDiv_obj, path2h5,
                                  'valid', False, (480, 640), scale=0.5)
    ...
    testObj = DataLoader_riteyes(dataDiv_obj, path2h5,
                                 'test', False, (480, 640), scale=0.5)
